...
raw_text = r'''SIC CODE DESCRIPTION
...
for line in lines:
    line = line.strip()
    if not line or line == 'List of SIC codes' or line == 'SIC CODE DESCRIPTION':
        continue
    
    match = re.match(r'^([0-9]{4,5})\s+(.+)$', line)
    if match:
        if current_item:
            sic_codes.append(current_item)
        current_item = {
            'code': match.group(1),
...
with open(os.path.join(data_dir, 'sic_codes.json'), 'w') as f:
    json.dump(sic_codes, f, indent=4)

print(f'Parsed {len(sic_codes)} SIC codes.')
