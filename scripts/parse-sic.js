...
const rawText = `SIC CODE DESCRIPTION
...
played
46439 Wholesale of radio, television goods & electrical household appliances (other than records, tapes,
CD's & video tapes and the equipment used for playing them)
...
47749 Retail sale of medical and orthopaedic goods in specialised stores (not incl. hearing aids) not
...
01629 Support activities for animal production (other than farm animal boarding and care) not elsewhere
classified
...
for (let i = 0; i < lines.length; i++) {
    ...
    if (!line || line === 'List of SIC codes' || line === 'SIC CODE DESCRIPTION') {
        continue;
    }

    // Match code and start of description
    // Most lines are "12345 Description text"
    const match = line.match(/^([0-9]{4,5})\s+(.+)$/);
    if (match) {
        if (currentItem) {
            sicCodes.push(currentItem);
        }
        currentItem = {
            code: match[1],
        ...
        // If it doesn't match the start pattern, it might be a continuation of the previous description
        // (like lines that wrap, e.g., "classified" or "materials")
...
// Generate the sic_codes.json file
const outputPath = path.join(__dirname, '../data', 'sic_codes.json');
const dataDir = path.dirname(outputPath);
if (!fs.existsSync(dataDir)) {
    fs.mkdirSync(dataDir, { recursive: true });
}

fs.writeFileSync(outputPath, JSON.stringify(sicCodes, null, 4));
console.log('Parsed ' + sicCodes.length + ' SIC codes.');
